	...
	t.Run("BedrockSecretsHidden", func(t *testing.T) {
		...
		_, err := client.CreateAIProvider(ctx, codersdk.CreateAIProviderRequest{
			...
			Settings: codersdk.AIProviderSettings{
				BedrockRegion:          "us-east-1",
				BedrockModel:           "anthropic.claude-3-5-sonnet",
				BedrockAccessKey:       "AKIA-leak", //nolint:gosec // test fixture, not a real credential
...
func TestAIProviderKeysCRUD(t *testing.T) {
	...
	t.Run("CreateListDelete", func(t *testing.T) {
		...
		// A fresh non-Bedrock provider starts with no keys.
		keys, err := client.AIProviderKeys(ctx, provider.Name)
		require.NoError(t, err)
		...
		// List returns both keys, ordered by created_at ASC, with the
		// plaintext api_key omitted.
		keys, err = client.AIProviderKeys(ctx, provider.Name)
		require.NoError(t, err)
		...
		require.Equal(t, second.ID, keys[0].ID)
	})
	...
	t.Run("BedrockProviderRejected", func(t *testing.T) {
		...
		provider, err := client.CreateAIProvider(ctx, codersdk.CreateAIProviderRequest{
			...
			Settings: codersdk.AIProviderSettings{
				BedrockRegion:          "us-east-1",
				BedrockModel:           "anthropic.claude-3-5-sonnet",
				BedrockAccessKey:       "AKIA-test", //nolint:gosec // test fixture, not a real credential
				BedrockAccessKeySecret: "bedrock-test-secret",
			},
		})
		require.NoError(t, err)
	...
	t.Run("DeleteCrossProviderForbidden", func(t *testing.T) {
		...
		// The key is still attached to provider A.
		keys, err := client.AIProviderKeys(ctx, providerA.Name)
		require.NoError(t, err)
		require.Len(t, keys, 1)
	...
	t.Run("KeyResponseHidesSecret", func(t *testing.T) {
		...
		// Inspect the raw HTTP body for the list endpoint. The api_key
		// field is intentionally omitted from the response shape, so the
		// plaintext should never appear over the wire.
		res, err := client.Request(ctx, http.MethodGet, "/api/v2/ai/providers/keys-secret/keys", nil)
		require.NoError(t, err)
		defer res.Body.Close()
		require.Equal(t, http.StatusOK, res.StatusCode)
		bodyBytes, err := io.ReadAll(res.Body)
	...
	t.Run("NonOwnerForbidden", func(t *testing.T) {
		...
		_, err = memberClient.AIProviderKeys(ctx, provider.Name)
		require.Error(t, err)
		var sdkErr *codersdk.Error
		require.ErrorAs(t, err, &sdkErr)
		require.Equal(t, http.StatusForbidden, sdkErr.StatusCode())
	...
	t.Run("ProviderNotFound", func(t *testing.T) {
		...
		//nolint:gocritic // Owner role is the audience for this endpoint.
		_, err := client.AIProviderKeys(ctx, "missing")
		require.Error(t, err)
		var sdkErr *codersdk.Error
		require.ErrorAs(t, err, &sdkErr)
		require.Equal(t, http.StatusNotFound, sdkErr.StatusCode())
