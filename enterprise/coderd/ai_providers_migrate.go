...
func (d desiredAIProvider) canonical() canonicalAIProvider {
	...
	if d.Bedrock != nil {
		...
		c.BedrockAccessKey = d.Bedrock.AccessKey
		c.BedrockAccessSecret = d.Bedrock.AccessKeySecret
	}
	return c
}
...
func providersFromEnv(cfg codersdk.AIBridgeConfig) ([]desiredAIProvider, error) {
	...
	if hasAnthropicKey || hasLegacyBedrock {
		...
		if hasLegacyBedrock {
			dp.Bedrock = &codersdk.AIProviderBedrockSettings{
				...
				AccessKey:       cfg.LegacyBedrock.AccessKey.String(),
				AccessKeySecret: cfg.LegacyBedrock.AccessKeySecret.String(),
			}
		}
	...
	for _, p := range cfg.Providers {
		...
		if errs := validateAIProviderName(name); len(errs) > 0 {
			return nil, xerrors.Errorf("invalid AI provider name %q: %s", name, errs[0].Detail)
		}
		...
		if dp.Type == database.AiProviderTypeAnthropic {
			...
			if len(p.BedrockAccessKeys) > 0 {
				bedrock.AccessKey = p.BedrockAccessKeys[0]
			}
			if len(p.BedrockAccessKeySecrets) > 0 {
				bedrock.AccessKeySecret = p.BedrockAccessKeySecrets[0]
			}
			...
			isBedrock = bedrock.AccessKey != "" || bedrock.AccessKeySecret != ""
			if isBedrock {
				dp.Bedrock = &bedrock
			}
		}
	...
	return db.InTx(func(tx database.Store) error {
		...
		for _, dp := range desired {
			...
			case found && existing.Deleted:
				...
				continue
			case found:
				existingSettings, err := decodeAIProviderSettings(existing.Settings)
				if err != nil {
